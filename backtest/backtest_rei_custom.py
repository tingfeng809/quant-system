...
from backtest.rei_custom_strategy import REICustomStrategy
...
def run_backtest(stock_list, strategy, start_date, end_date, max_stocks=None):
    """运行回测"""
    loader = DataLoader()
...
def main():
    ...
    df = run_backtest(stock_list, strategy, start_date, end_date, max_stocks=1000)
    ...
    output_file = os.path.join(os.path.dirname(__file__), '..', 'output', 
                              f'rei_backtest_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
